...
export default function Sidebar() {
  ...
  const sidebarLinkClass = (path) => 
    `flex items-center gap-2.5 px-3 py-2 text-sm transition-all rounded-sm ${isActive(path) ? 'bg-gold/15 text-gold' : 'text-muted hover:text-white hover:bg-white/5'}`

  return (
    <aside className="sidebar hidden md:flex flex-col w-60 min-h-screen fixed left-0 top-0 pt-20 pb-6 px-3 overflow-y-auto sidebar-scrollbar"
      ...
      <div className="mt-2 border-t border-white/5 pt-3">
        {isAdmin && (
          <>
            <Link to="/admin" className={sidebarLinkClass('/admin')}>
              <span>🔧</span><span>Administration</span>
            </Link>
            <Link to="/admin/users" className={sidebarLinkClass('/admin/users')}>
              <span>👥</span><span>Utilisateurs</span>
            </Link>
            <Link to="/admin/finance" className={sidebarLinkClass('/admin/finance')}>
              <span>💰</span><span>Finance</span>
            </Link>
            <Link to="/admin/marketing" className={sidebarLinkClass('/admin/marketing')}>
