    ...
    <AppLayout>
      ...
      <div className="flex flex-col h-[calc(100dvh-80px)] max-h-[880px]">
        ...
        <div className="flex items-end justify-between mb-5 flex-shrink-0">
          ...
          {level && (
            <span
              className="font-mono text-[9px] tracking-[0.2em] px-3 py-1.5 rounded-sm border"
              style={{
                color:       langAccent.color,
                background:  `${langAccent.color}18`,
                borderColor: `${langAccent.color}33`,
              }}
            >
              {level} · {langInfo?.name}
            </span>
        ...
        <div
          className="flex flex-wrap gap-2 mb-4 p-3 rounded-card flex-shrink-0"
          style={{
            ...
            {languages.map(l => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-sm text-sm font-medium transition-all duration-250"
                  style={sel ? {
            ...
            {accessibleModes.map(m => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-1 px-3 py-1.5 rounded-sm text-xs transition-all duration-250"
                  style={sel ? {
        ...
        {/* ── Zone de chat ──────────────────────────────────── */}
        <div
          className="flex-1 overflow-y-auto sidebar-scrollbar rounded-card p-4 mb-3 space-y-5"
          style={{
        ...
        >
          {messages.map((msg, i) => (
            <ChatBubble key={i} msg={msg} langAccent={langAccent} />
          ))}

          {/* Typing indicator */}
          {aiLoading && (
            <div className="flex items-end gap-2.5">
              <AIAvatar isSpeaking={false} langAccent={langAccent} />
              <div
        ...
        {(isListening || isSpeaking) && (
          <div className="flex items-center justify-center gap-6 py-2 mb-1 flex-shrink-0">
            {isListening && <VoiceWaveIndicator label={`Écoute · ${langInfo?.name}`} color="#EF4444" />}
            {isSpeaking  && <VoiceWaveIndicator label="L'IA parle..." color={langAccent.color} />}
          </div>
        )}

        {/* ── Zone de saisie ────────────────────────────────── */}
        <div
          className="flex gap-2 items-end p-2 rounded-card flex-shrink-0"
          style={{
...
function AIAvatar({ isSpeaking, langAccent }) {
  return (
    <div
      ...
      style={{
        ...
        boxShadow:  isSpeaking
          ? `0 0 16px ${langAccent.glow}, 0 0 32px ${langAccent.glow}`
          : '0 2px 8px rgba(0,0,0,0.4)',
        animation:  isSpeaking ? 'speak-pulse 2s ease-in-out infinite' : 'none',
      }}
    >
      ◎
    </div>
...
function VoiceWaveIndicator({ label, color }) {
  return (
